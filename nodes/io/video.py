...
class FSLoadVideoFromURL:
    ...
    def load_video_from_url(
        ...
        preview = {
            "ui": {
                "video": [
                    {"filename": os.path.basename(video_url), "type": "input", "url": video_url}
                ]
            }
        }

        # Load the video using OpenCV
        result = FSLoadVideo().load_video(
            temp_video_path, skip_first_frames, select_every_nth, prompt, extra_pnginfo
        )
        ...
        if isinstance(result, dict) and "result" in result:
            ...
            # result is a tuple/list, get the first element
            image_tensor = result[0]  # type: ignore
                ...
                if format == "mp4":
                    ...
                    if working_codec is None:
                        ...
                        output_path = os.path.join(
                            output_dir, f"{os.path.splitext(save_filename)[0]}.{format}"
                        )
                        save_filename = f"{filename}_{counter:05}.{format}"

                    fourcc = cv2.VideoWriter_fourcc(*working_codec)  # type: ignore
    ...
    def _encode_with_ffmpeg(self, frame_dir, output_path, fps, format, quality):
        ...
        if format == "mp4":
            ...
            if encoder in ("libx264", "h264_nvenc", "h264_qsv"):
                ...
                cmd += ["-q:v", str(int(31 - (quality / 100.0 * 30)))]
        elif format == "webm":
            cmd = [
                "ffmpeg",
                "-y",
            ...
            cmd = [
                ...
                codec,
                "-q:v",
                str(int(31 - (quality / 100.0 * 30))),  # Quality parameter
                "-threads",
                "0",
            ]
