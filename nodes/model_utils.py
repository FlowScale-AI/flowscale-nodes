...
class LoadModelFromURL:
    ...
    def load_model_from_url(self, model_url, path):
        ...
        if os.path.isdir(COMFYUI_MODELS_DIR):
            ...
            logger.warning(
                f"COMFYUI_MODELS_DIR '{COMFYUI_MODELS_DIR}' is not a directory, using provided path as is"
            )
            full_path = path
        ...
        if "huggingface.co" in model_url:
            if not HUGGINGFACE_API_KEY:
                raise Exception(
                    "HUGGINGFACE_API_KEY is not set. Please set it in the environment variables."
                )
            cmd = [
                "wget",
                "--header",
                f"Authorization: Bearer {HUGGINGFACE_API_KEY}",
                "--progress=dot:mega",
                model_url,
                "-O",
                full_path,
            ...
            if not CIVITAI_API_KEY:
                raise Exception(
                    "CIVITAI_API_KEY is not set. Please set it in the environment variables."
                )
            modified_download_url = (
                model_url + ("&" if "?" in model_url else "?") + "token=" + CIVITAI_API_KEY
            )
            cmd = ["wget", "--progress=dot:mega", modified_download_url, "-O", full_path]
        else:
            cmd = [
                "wget",
                "--progress=dot:mega",
