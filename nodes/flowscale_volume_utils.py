...
class SaveModelToFlowscaleVolume:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "path_in_volume": (
                    "STRING",
                    {"multiline": False, "placeholder": "/path/to/model, e.g. /loras"},
                ),
    ...
    def _create_root_folder(self, model_type, webhook_url=""):
        ...
        try:
            ...
            raise Exception(f"Failed to create folder in Flowscale volume: {e}") from e
    ...
    def _upload_single_model(
        ...
        try:
            ...
            logger.info(f"Response from Flowscale: {response_json}")
            if response_json.get("status") == "error":
                error_message = response_json.get("data", "Unknown error")
                if len(webhook_url.strip()) > 0:
                    httpx.post(webhook_url, json={"error": error_message})
                if error_message == "Model already exists in volume":
                    return download_url, "model_exists"
                raise Exception(f"Error from Flowscale API: {error_message}")
            ...
            if not file_id:
                if len(webhook_url.strip()) > 0:
                    httpx.post(
                        webhook_url,
                        json={
                            "error": "Something went wrong while uploading the model to Flowscale volume"
                        },
                    )
                raise Exception(
                    "Something went wrong while uploading the model to Flowscale volume"
                )
        except httpx.RequestError as e:
            if len(webhook_url.strip()) > 0:
                httpx.post(webhook_url, json={"error": str(e)})
            raise Exception(f"Failed to upload model to Flowscale volume: {e}") from e
        ...
        if response.status_code != 200:
            ...
            raise Exception(
                f"Failed to upload model to Flowscale volume: {response_json.get('data')}"
            )
    ...
    def upload_model_to_flowscale_volume(
        self, model_type, path_in_volume, download_url, api_key="", model_name="", webhook_url=""
    ):
