...
async def list_models(request):
    ...
    try:
        ...
        all_items = os.listdir(
            MODELS_DIR if not model_folder else os.path.join(MODELS_DIR, model_folder)
        )
        models = []
        ...
        for item_name in all_items:
            ...
            models.append({"name": item_name, "type": item_type})
...
async def upload_model(request):
    ...
    try:
        ...
        if (
            model_file_field.filename
            and model_file_field.filename.endswith(".safetensors")
            and not model_name.endswith(".safetensors")
        ):
            logger.debug("Adding .safetensors extension to model name")
