...
class MultiFileLoaderNode:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "filter_extensions": (
                    "STRING",
                    {"default": "", "placeholder": "jpg,png,pdf (comma separated)"},
                ),
    ...
    def process_files(
        self, file_list, label="Multi-File Uploader", filter_extensions="", extract_metadata=False
    ):
        """
        ...
        if filter_extensions:
            extensions = [
                ext.strip().lower() for ext in filter_extensions.split(",") if ext.strip()
            ]
            if extensions:
                files = [
                    f for f in files if any(f.lower().endswith(f".{ext}") for ext in extensions)
                ]
    ...
    def _extract_file_metadata(self, file_path):
        ...
        if mime_type and mime_type.startswith("image/"):
            try:
                ...
                print(f"Warning: Error extracting image metadata: {e}")
        ...
        # Get video info
            ...
            try:
                ...
                print("Warning: OpenCV not installed, cannot extract video metadata")
            except Exception as e:
                print(f"Warning: Error extracting video metadata: {e}")
